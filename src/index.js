...
const _connect = function(box, maxRetries) {
  ...
  const connectionOptions = connOptsParser.parse(
    box.integrations.mqtt.connectionOptions
  );
  ...
  return new Promise(function(resolve, reject) {
    client.on('error', function(err) {
      ...
      if (errRetries === 0) {
        ...
        return reject(
          `connection closed after 5 retries because of ${err}. Retry after ${
            RETRY_AFTER_MINUTES
          } minutes`
        );
      }
    ...
    client.on('close', function() {
      ...
      if (closeRetries === 0) {
        ...
        return reject(
          `connection closed after 5 retries. Retry after ${
            RETRY_AFTER_MINUTES
          } minutes`
        );
      }
        ...
        .then(function(client) {
          ...
          client.on('message', function(topic, message) {
            ...
            Promise.all([
              ...
              handler.decodeMessage(msgStr, decodeOptions)
            ])
              ...
              .then(function({ ok, n }) {
                if (ok === n) {
                  log.info(
                    `received, decoded and saved mqtt message for box ${
                      box._id
                    }`
                  );
                }
